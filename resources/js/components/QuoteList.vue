<template>
  <div class="">
    ...
    <ul v-else class="">
      <li v-for="quote in paginatedQuotes" :key="quote.id" class="">
        <p>"{{ quote.quote }}"</p>
        <router-link :to="'/quotes-ui/' + quote.id" class="">See details</router-link>
      </li>
    </ul>

    <div class="">
      <button @click="page--" :disabled="page === 1" class="">Before</button>
      <span class="">Page {{ page }}</span>
      <button @click="page++" :disabled="page >= maxPages" class="">Next</button>
    </div>
  </div>
</template>
